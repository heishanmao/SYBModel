...
import PlotLogisticsRoutes as FIG
...
Rates = [0.5, 0.8, 1.2, 1.0, 1.5, 2.0, 3.0]  #Truck Rates Changed
ExpRES = pd.DataFrame()
for index, r in enumerate(Rates):
    # @Datasets normal Scenarios
    # Country_Elevator to Stream_Elevator by Trucks  @(c, s)
    Cost_Country_Stream = pd.read_csv(root + '\Data\Cost\CostToStreamByTruck.csv', index_col=0).to_numpy()*r

    # Country_Elevator to Rail_Elevator by Trucks  @(c, r)
    Cost_Country_Rail = pd.read_csv(root + '\Data\Cost\CostToRailByTruck.csv', index_col=0).to_numpy()*r
    ...
    Cost_Country_Facility = pd.read_csv(root + '\Data\Cost\CostToFacility.csv', index_col=0, usecols=['Name', 'Facility']).T.to_numpy()[0]*r
...
ExpRES.to_csv(root+'\Exp\Exp2_1_Truck.csv')