...
# @Main    : Zheng@utk.edu
# @File    : Distance.py
        # a spider to get distance from river & rail to exports
# @Software: PyCharm
...
if __name__=='__main__':
    ...
    # import Data
    originLocation = pd.read_csv("..\Locations\LargerRiverElevators.csv")
    ...
    res = pd.DataFrame(index=originLocation['Name'], columns=destinationLocation['Ports'])
    for i in range(len(originLocation)):
        for j in range(len(destinationLocation)):
            res.iloc[i,j] = GetDistance(originLocation.iloc[i,1], destinationLocation.iloc[j, 3])
            print(originLocation.iloc[i,1],destinationLocation.iloc[j,1], res.iloc[i,j])
    res.to_csv('.\DistanceRiverToPorts.csv')
