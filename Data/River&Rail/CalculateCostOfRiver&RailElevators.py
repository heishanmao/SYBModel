...
River_Export_Distance = pd.read_csv('.\DistanceRiverToPorts.csv', index_col = 0)
River_Export_Distance['min_val'] = River_Export_Distance.min(axis=1)
Rail_Export_Distance = pd.read_csv('.\DistanceRailToPorts.csv', index_col = 0)
...
for i in range(Cost_River_Export.shape[0]):
    for j in range(Cost_River_Export.shape[1]):
        Cost_River_Export.iloc[i,j] = NormalizateData(Cost_River_Export.iloc[i,j], 50, 90, OMax_River, OMin_River)
    
Cost_River_Export.to_csv('..\Cost\CostStreamToExport.csv')
...
for i in range(Cost_Rail_Export.shape[0]):
    for j in range(Cost_Rail_Export.shape[1]):
        Cost_Rail_Export.iloc[i, j] = NormalizateData(Cost_Rail_Export.iloc[i, j], 40, 80, OMax_Rail, OMin_Rail)

Cost_Rail_Export.to_csv('..\Cost\CostRaiToExport.csv')
