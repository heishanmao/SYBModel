...
Distance = pd.read_csv('.\DistanceExportToImport.csv', index_col = 0)
...
for i in range(Cost.shape[0]):
    for j in range(Cost.shape[1]):
        Cost.iloc[i,j] = NormalizateData(Cost.iloc[i,j], 100, 120, OMax, OMin)
    
Cost.to_csv('..\Cost\CostExportToImport.csv')
