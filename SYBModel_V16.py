...
class GCAM_SYB():
    def __init__(self, model_name, year, china_demand, **kwargs):
        # Parameters
    ...
    def _build_model(self, lp_to_file=False):
        ...
        # self.Domestic_Price = self.model.addVar(vtype=GRB.CONTINUOUS, name='Domestic_Price')
        # self.Global_Price = self.model.addVar(vtype=GRB.CONTINUOUS, name='Global_Price')
        # self.Slack_Climate = self.model.addVar(1, vtype=GRB.BINARY, name='Slack_Climate')  # Slack Vars
        # self.Slack_Tariff = self.model.addVar(1, vtype=GRB.BINARY, name='Slack_Tariff')
        ...
        # 6 Z_Export_Import
        self.model.addConstr(quicksum(self.Z_Export_Import[k, m] for k in range(self.Num_Export_Terminals) for m in range(self.Num_Import_Terminals)) >= self.Demand_China)
        ...
        # Objective
        obj = LinExpr()
        obj += quicksum(self.Cost_Export_Import[k, m] * self.Z_Export_Import[k, m] for k in range(self.Num_Export_Terminals) for m in range(self.Num_Import_Terminals))
        obj += quicksum(self.Cost_Country_Facility[c] * self.X_Facility[c] for c in range(self.Num_Country_Elevators))
        # obj += quicksum(self.global_price - self.Cost_Export_Import[k, m] * self.Z_Export_Import[k, m] for k in range(self.Num_Export_Terminals) for m in range(self.Num_Import_Terminals))
        # obj += quicksum(self.domestic_price - self.Cost_Country_Facility[c] * self.X_Facility[c] for c in range(self.Num_Country_Elevators))
        obj += quicksum(self.Cost_Country_Stream[c, s] * self.X_Country_Stream[c, s] for c in range(self.Num_Country_Elevators) for s in range(self.Num_Stream_Elevators))
        obj += quicksum(self.Cost_Country_Rail[c, r] * self.X_Country_Rail[c, r] for c in range(self.Num_Country_Elevators) for r in range(self.Num_Rail_Elevators))
        obj += quicksum(self.Cost_Stream_Export[s, k] * self.Y_Stream_Export[s, k] for s in range(self.Num_Stream_Elevators) for k in range(self.Num_Export_Terminals))
        obj += quicksum(self.Cost_Rail_Export[r, k] * self.Y_Rail_Export[r, k] for r in range(self.Num_Rail_Elevators) for k in range(self.Num_Export_Terminals))
        obj += quicksum(self.holding_cost * self.Inventory_Country[c] for c in range(self.Num_Country_Elevators))
        obj += quicksum(self.Yield_Country[c, 4] * (self.IRR_lo * self.Framer_Decision[c, 0] + self.IRR_hi * self.Framer_Decision[c, 1] + self.RFD_lo * self.Framer_Decision[c, 2] +
...
if __name__ == '__main__':
    ...
    instacne = GCAM_SYB(scenario, year, china.quantity*0.5,
                        ...
                        truck_rate=1, barge_rate=1, rail_rate=1, ocean_rate=8)