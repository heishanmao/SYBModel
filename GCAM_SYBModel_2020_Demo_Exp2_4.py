...
Rates = [0.5, 0.8, 1.2, 1.0, 1.5, 2.0, 3.0]  #Ocean Rates Changed
ExpRES = pd.DataFrame()
for index, r in enumerate(Rates):
    ...
    Cost_Export_Import = pd.read_csv(root + '\Data\Cost\CostExportToImport.csv', index_col=0).to_numpy()*r
...
ExpRES.to_csv(root+'\Exp\Exp2_4_Ocean.csv')