...
class GCAM_SYB():
    ...
    def _load_quantity(self):
        def total_production(x):
            d1 = sum(x[:,0] * x[:,4]) / self.china_demand
            d2 = sum(x[:,1] * x[:,4]) / self.china_demand
            d3 = sum(x[:,2] * x[:,4]) / self.china_demand
            d4 = sum(x[:,3] * x[:,4]) / self.china_demand

            beta = math.ceil(d4)

            x[:, 4] = x[:, 4] / beta

            d1_2 = sum(x[:, 0] * x[:, 4]) / self.china_demand
            d2_2 = sum(x[:, 1] * x[:, 4]) / self.china_demand
            d3_2 = sum(x[:, 2] * x[:, 4]) / self.china_demand
            d4_2 = sum(x[:, 3] * x[:, 4]) / self.china_demand
        ...
        self.Yield_Country = total_production(self.Yield_Country)
    ...
    def _build_model(self, lp_to_file=False):
        ...
        # Variables
        self.Framer_Decision = self.model.addVars(self.Num_Country_Elevators, 4, lb=0, ub=1, vtype=GRB.CONTINUOUS, name='Framer_Decision')     # 4 Farmer decisions
        ...
        # 2 Supplyment
        for c in range(self.Num_Country_Elevators):
            self.model.addConstr(self.Supply_Country[c] + self.Inventory_Country[c] - quicksum(self.X_Country_Stream[c, s] for s in range(self.Num_Stream_Elevators)) - quicksum(self.X_Country_Rail[c, r] for r in range(self.Num_Rail_Elevators))
                                 - self.X_Facility[c] - self.I_Country[c] == 0)
